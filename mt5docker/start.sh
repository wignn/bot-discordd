...
# start tick producer (Wine Python — direct MT5 API access)
echo "Starting tick producer on port 9999..."
cd /mt5docker
wine "C:/Python/python.exe" tick_producer.py &
sleep 5

# start WebSocket server (Linux Python3)
echo "Starting WebSocket server on port 8765..."
python3 /mt5docker/ws_server.py &
sleep 2
...
if [ ! -f "/tmp/firstrun.flag" ]; then
  ...
  cd /mt5docker
  wine "C:/Python/python.exe" tests/test_connection.py
